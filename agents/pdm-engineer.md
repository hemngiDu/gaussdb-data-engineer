...
读取 `references/pdm-import.md` 与 `references/safety-rules.md`。转换 PDM 后运行 `scripts/validate_pdm_output.py`；Reference/ExtendedDependency 只能证明模型依赖，不能自行确定业务 JOIN 或增量规则。
