...
description: 华为云GaussDB数据开发专用技能。按数据工程师yufeng的固定风格生成SQL代码（DDL建表、DML清洗、ETL加工）。触发场景：(1)用户提供数据模型或表结构要求生成SQL (2)用户给出现有代码要求按风格改写 (3)用户给出Power Designer .pdm文件要求生成DDL (4)需要按分层架构(irpt→dwi→dws→ads)写SQL的任务
---
