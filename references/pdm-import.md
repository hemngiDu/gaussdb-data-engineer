...
脚本读取表/字段 `Code`、`Name`、`DataType`、`Length`，以及表级 Reference/ReferenceJoin 和 ExtendedDependency。保留已知类型和精度（例如 BIGINT 与 DECIMAL(20,6)）；未知类型标记 NEED_CONFIRM。PDM 描述文本不直接作为 WHERE 条件执行。
